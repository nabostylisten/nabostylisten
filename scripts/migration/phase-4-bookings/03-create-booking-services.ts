...
export async function createBookingServices(): Promise<void> {
  ...
  try {
    logger.info('Starting Phase 4 Step 3: Create Booking-Service Relationships');
    ...
    for (const booking of bookings) {
      if (booking.service_ids && booking.service_ids.length > 0) {
        ...
        logger.debug(`Found ${booking.service_ids.length} services from JSON for booking ${booking.id}`);
      }
    }
    ...
    try {
      ...
      logger.warn('Could not load service ID mapping from Phase 3, skipping junction table processing');
    }
    ...
    for (const junctionRecord of junctionRecords) {
      ...
      if (!mappedServiceId) {
        logger.warn(`Skipping junction record: MySQL service ID ${junctionRecord.service_id} not found in Phase 3 mapping`);
        continue;
      }
    ...
    logger.info(`Found ${fromJunctionTable} booking-service relationships from junction table`);
    ...
    for (const relationship of allRelationships) {
      ...
      if (uniqueRelationships.has(key)) {
        duplicates++;
        logger.debug(`Duplicate relationship found: ${key} (${relationship.source})`);
      } else {
    ...
    logger.info(`Final ${finalRelationships.length} unique booking-service relationships to create (${duplicates} duplicates removed)`);
    ...
    for (const relationship of finalRelationships) {
      try {
        const bookingServiceData: Database["public"]["Tables"]["booking_services"]["Insert"] = {
        ...
        if (createResult.success) {
          ...
          logger.debug(`✅ Created booking-service relationship: ${relationship.booking_id} -> ${relationship.service_id} (${relationship.source})`);
        } else {
          results.push({
            booking_id: relationship.booking_id,
            service_id: relationship.service_id,
          ...
          logger.error(`❌ Failed to create booking-service relationship ${relationship.booking_id} -> ${relationship.service_id}: ${createResult.error}`);
        }
        ...
        logger.error(`❌ Exception creating booking-service relationship ${relationship.booking_id} -> ${relationship.service_id}:`, error);
      }
    }
    ...
    if (failed > 0) {
      logger.warn(`⚠️  ${failed} booking-service relationships failed to create. Check logs for details.`);
    }
