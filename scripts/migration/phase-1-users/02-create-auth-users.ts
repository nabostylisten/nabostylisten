...
async function main() {
  ...
  try {
        ...
        try {
          ...
          const authResult = await database.createAuthUser({
            ...
            user_metadata: {
              ...
              role: user.role,
              migration_source: user.source_table === 'buyer' ? 'mysql_buyer' : 'mysql_stylist',
              original_id: user.original_id
            }
          });
    ...
    writeFileSync(authResultsPath, JSON.stringify({
      metadata: {
    ...
    const successfulResults = results.filter(r => r.success && r.supabase_user_id);
    ...
    logger.stats('Auth User Creation Summary', {
      ...
      'Errors': errors,
      'Success Rate': `${Math.round((created / (users.length - skipped)) * 100)}%`,
