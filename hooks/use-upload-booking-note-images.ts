...
interface UploadBookingNoteImagesParams {
    bookingId: string;
    noteId: string;
    files: File[];
}

export const useUploadBookingNoteImages = () => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    compressedFile = await imageCompression(file, {
                        maxSizeMB: bucketConfigs["booking-note-media"].maxSize /
                    ...
                    const validation = validateFile(
                        file,
                        allowedTypes,
                        bucketConfigs["booking-note-media"].maxSize,
                    );
                    if (validation) {
