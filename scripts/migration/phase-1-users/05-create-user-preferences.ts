...
async function main() {
  ...
  try {
    ...
    logger.info(`Processing ${usersToProcess.length} users for preferences (skipped ${skippedUsers})`);
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      const preferencesBatch: Database['public']['Tables']['user_preferences']['Insert'][] = batch.map(user => {
      ...
      try {
        ...
        const batchResult = await database.batchCreate('user_preferences', preferencesBatch as Record<string, unknown>[]);

        if (batchResult.success) {
          logger.debug(`Successfully created batch ${batchIndex + 1}/${totalBatches}: ${batchResult.inserted} user preferences`);
          ...
          for (const user of batch) {
            ...
            const userPreferences: Database['public']['Tables']['user_preferences']['Insert'] = {
              user_id: supabaseId,
              newsletter_subscribed: prefs.newsletter_subscribed,
              marketing_emails: prefs.marketing_emails,
              promotional_sms: prefs.promotional_sms,
            ...
            const individualResult = await database.createUserPreferences(userPreferences);
            ...
            if (individualResult.success) {
              ...
              logger.error(`Failed to create user preferences for ${user.email}`, individualResult.error);
              results.push({
                original_id: user.original_id,
                supabase_id: supabaseId,
                email: user.email,
    ...
    // Save results
    const preferencesResultsPath = join(tempDir, 'user-preferences-created.json');
    writeFileSync(preferencesResultsPath, JSON.stringify({
      metadata: {
    ...
    logger.stats('Phase 1 User Migration COMPLETE', {
      ...
      'Success Rate': stats.created_auth_users > 0 ? `${Math.round((stats.created_profiles / stats.created_auth_users) * 100)}%` : '0%',
      'Overall Status': stats.errors === 0 ? '✅ SUCCESS' : (stats.created_profiles > 0 ? '⚠️  PARTIAL SUCCESS' : '❌ FAILED')
    });

    // Save Phase 1 completion marker
    const completionMarkerPath = join(tempDir, 'phase-1-completed.json');
    writeFileSync(completionMarkerPath, JSON.stringify({
    ...
    logger.info('Next steps: Users can now login using OTP with their existing emails');
