...
async function main() {
  ...
  try {
    ...
    logger.info("Filtered active users", {
      ...
      filtered_out: (buyers.length - activeBuyers.length) +
        (stylists.length - activeStylists.length),
    });
    ...
    const hasValidationErrors = buyerErrors.length > 0 ||
      stylistErrors.length > 0;
    ...
    const duplicates = deduplicator.findDuplicateEmails(
      activeBuyers,
      activeStylists,
    );
    ...
    const consolidatedUsers = deduplicator.consolidateUsers(
      activeBuyers,
      activeStylists,
      duplicates,
    );
    ...
    const consolidationErrors = validator.validateConsolidatedUsers(
      consolidatedUsers,
    );
    ...
    if (consolidationErrors.length > 0) {
      ...
      throw new Error(
        `Consolidated user validation failed with ${consolidationErrors.length} errors`,
      );
    }
    ...
    const stats: UserMigrationStats = {
      ...
      merged_accounts: duplicates.filter((d) =>
        d.resolution !== "create_separate"
      ).length,
      skipped_records: buyers.length + stylists.length -
      ...
      errors: buyerErrors.length + stylistErrors.length +
        consolidationErrors.length,
    };
    ...
    logger.stats("User Extraction Summary", {
      ...
      "Customers": consolidatedUsers.filter((u) =>
        u.role === "customer"
      ).length,
