...
export async function POST(request: NextRequest) {
  try {
    ...
    if (bookingsError) {
      console.error(
        "[BOOKING_REMINDERS] Error fetching bookings:",
        bookingsError,
      );
    ...
    console.log(
      `[BOOKING_REMINDERS] Found ${bookings.length} bookings to process`,
    );
    ...
    for (const booking of bookings) {
      try {
        ...
        if (!canSendReminder) {
          console.log(
            `[BOOKING_REMINDERS] Skipping reminder for booking ${booking.id} - customer preferences disabled`,
          );
          continue;
        }
        ...
        if (!booking.customer?.email || !booking.stylist?.full_name) {
          console.log(
            `[BOOKING_REMINDERS] Skipping booking ${booking.id} - missing customer email or stylist name`,
          );
          continue;
        }
        ...
        if (emailError) {
          console.error(
            `[BOOKING_REMINDERS] Failed to send reminder for booking ${booking.id}:`,
            emailError,
          );
          errorsCount++;
        } else {
          console.log(
            `[BOOKING_REMINDERS] Sent reminder for booking ${booking.id} to ${booking.customer.email}`,
          );
          emailsSent++;
        }
      } catch (error) {
        console.error(
          `[BOOKING_REMINDERS] Error processing booking ${booking.id}:`,
          error,
        );
        errorsCount++;
      }
    ...
    console.log(
      `[BOOKING_REMINDERS] Completed: ${emailsSent} emails sent, ${errorsCount} errors`,
    );
