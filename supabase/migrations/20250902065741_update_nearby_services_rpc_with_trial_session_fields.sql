drop function if exists "public"."nearby_services"(lat double precision, long double precision, radius_km double precision, search_term text, category_ids text[], min_price_ore integer, max_price_ore integer, at_customer_place boolean, at_stylist_place boolean, stylist_ids text[], sort_by text);
