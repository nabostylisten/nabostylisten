...
async function main() {
  ...
  try {
    ...
    // Process all users ( logic removed for production migration)
    ...
    logger.info(
      `Processing ${stylistsToProcess.length} stylists (skipped ${skippedStylists})`,
    );
    ...
    const createStylistDetailsRecord = (
      user: ConsolidatedUser,
    ): Database["public"]["Tables"]["stylist_details"]["Insert"] => {
    ...
    logger.info(
      `Processing ${stylistsToProcess.length} stylists in batches of ${batchSize}`,
    );
    ...
    const batchResult = await processDatabaseBatches<
      ConsolidatedUser,
      StylistResult
    >(
      stylistsToProcess,
      ...
      async (users: ConsolidatedUser[]): Promise<StylistResult[]> => {
        ...
        if (batchResult.success) {
          ...
          throw new Error(
            `Batch insert failed: ${batchResult.errors.join("; ")}`,
          );
        }
      },
      {
    ...
    // Save results
    const stylistDetailsResultsPath = join(
      tempDir,
      "stylist-details-created.json",
    );
    writeFileSync(
      stylistDetailsResultsPath,
      JSON.stringify(
        {
          metadata: {
    ...
    logger.stats("Stylist Details Creation Summary", {
      ...
      "Errors": errors,
      "Success Rate": stylistsToProcess.length > 0
        ? `${Math.round((created / stylistsToProcess.length) * 100)}%`
        : "0%",
