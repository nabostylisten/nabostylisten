...
async function main() {
  ...
  try {
    ...
    let addressResults: { results: Array<{ success: boolean; original_id: string }> } = { results: [] };
    try {
      if (readFileSync(addressResultsPath, "utf-8")) {
        addressResults = JSON.parse(readFileSync(addressResultsPath, "utf-8"));
      }
    } catch {
      logger.warn("No addresses-created.json file found - no addresses were created in previous step");
    ...
    // Early exit if no addresses were created
    if (createdAddresses.size === 0) {
      logger.info("No addresses were created - skipping primary address flag updates");
      logger.success("Phase 2 Step 3 completed (no addresses to process)");
      return;
    }
    ...
    for (const buyer of buyers) {
      if (
        buyer.is_deleted || !buyer.default_address_id || !userMapping[buyer.id]
      ) {
        continue;
      }
    ...
    for (const stylist of stylists) {
      if (
        stylist.is_deleted || !stylist.default_address_id ||
        !userMapping[stylist.id]
      ) {
        continue;
      }
    ...
    logger.info(
      `Found ${primaryAddressUpdates.length} primary address updates to perform`,
    );
    ...
    for (const update of primaryAddressUpdates) {
      try {
        const result = await database.updateAddressPrimary(
          update.address_id,
          true,
        );
        ...
        if (result.success) {
          ...
          successful++;
          logger.debug(
            `Set primary address for ${update.user_email}: ${update.address_id}`,
          );
        } else {
          results.push({
            user_id: update.user_id,
          ...
          errors++;
          logger.error(
            `Failed to set primary address for ${update.user_email}`,
            result.error,
          );
        }
      } catch (error) {
        ...
        errors++;
        logger.error(
          `Exception setting primary address for ${update.user_email}`,
          error,
        );
      }
    ...
    // Save results
    const primaryAddressResultsPath = join(
      tempDir,
      "primary-addresses-updated.json",
    );
    writeFileSync(
      primaryAddressResultsPath,
      JSON.stringify(
        {
          metadata: {
            updated_at: new Date().toISOString(),
    ...
    logger.stats("Primary Address Update Summary", {
      ...
      "Errors": errors,
      "Success Rate": primaryAddressUpdates.length > 0
        ? `${Math.round((successful / primaryAddressUpdates.length) * 100)}%`
        : "0%",
      "Primary Addresses in Database": primaryCount,
      "Total Addresses": counts.addresses,
      "Status": errors === 0
        ? "✅ SUCCESS"
        : (successful > 0 ? "⚠️  PARTIAL SUCCESS" : "❌ FAILED"),
    });
